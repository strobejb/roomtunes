...
int main(int argc, char *argv[])
{
    ...
    QCommandLineOption spotifyLinkOption(QStringLiteral("spotify-link"),
    ...
    if (parser.isSet(browseOption)) {
        ...
        QObject::connect(&household, &Household::musicServicesChanged, &app, [&household, &app, browsedKeys]() {
            ...
            for (MusicService *service : household.services()) {
                ...
                QObject::connect(service, &MusicService::browseFinished, &app,
                                  [service](const QString &, bool ok, const QString &errorMessage,
    ...
    if (parser.isSet(browseIdOption)) {
        ...
        QObject::connect(&household, &Household::musicServicesChanged, &app, [&household, &app, objectId, probed]() {
            ...
            *probed = true;
            ...
            QObject::connect(library, &MusicService::browseFinished, &app,
                              [objectId](const QString &, bool ok, const QString &errorMessage,
                                        const QVariantList &items) {
                QTextStream out(stdout);
