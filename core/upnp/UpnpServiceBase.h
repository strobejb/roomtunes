...
{
public:
    UpnpServiceBase(QNetworkAccessManager *netMgr, const QString &device, int port,
                     QString controlPath, QString eventPath, QString serviceUrn, const char *name)
        : m_netMgr(netMgr)
        , m_controlPath(std::move(controlPath))
        , m_eventPath(std::move(eventPath))
        , m_action(std::move(serviceUrn))
        , m_name(name)
    {
    ...
    void setSid(const QString &sid) override { m_sid = sid; }
    const QString &sid() const override { return m_sid; }
    bool subscribed() const override { return !m_sid.isEmpty(); }
    const char *serviceName() const override { return m_name; }
    ...
    QNetworkReply *subscribe(const QString &localAddr, int port, int timeoutSeconds = 3600) override
    {
        ...
        const QString callback = QStringLiteral("<http://%1:%2/%3/notify>").arg(localAddr).arg(port).arg(QLatin1String(m_name));
