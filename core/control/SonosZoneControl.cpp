...
SonosZoneControl::SonosZoneControl(AVTransport &avTransport, RenderingControl &renderingControl,
    ...
    : m_avTransport(avTransport), m_renderingControl(renderingControl), m_contentDirectory(contentDirectory),
      m_queue(queue), m_roomNameProvider(std::move(roomNameProvider))
{
}
...
void SonosZoneControl::browseDetailed(QObject *context, const QString &objectId,
                                      std::function<void(bool, const BrowseResult &)> callback, int startingIndex,
